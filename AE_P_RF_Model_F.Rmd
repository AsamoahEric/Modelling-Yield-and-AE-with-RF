...
subtitle: "Random Forest Modelling for Maize AE_P (kg/kg) Prediction - Nested Cross Valdation approach for hyperparameter tuning and model evaluation"
---
...
setwd("C:/Users/Eric_Asamoah/OneDrive - Wageningen University & Research/FERARI PhD 1 - Eric Asamoah/Academic/Data/Scripts/4_RF_models/Final_RF_Models/AE_P")
```
...
Comp_dats <- subset(Compiled_Datasets, select = -c(Site_ID,Year_of_experiment,Latitude,Longitude,Grain_yield_kg_ha,AE_N,AE_K,N_kg_ha,P2O5_kg_ha,K2O_kg_ha,Plot_ID))

# Reorder columns in the compiled datasets
Comp_dats <- relocate(Comp_dats, AE_P, .after = Slope)
```
...
Comp_dats <- subset(Comp_dats, !is.na(Comp_dats$AE_P))
...
AE_P_data <- AE_P_data[sample(1:nrow(AE_P_data)),]
AE_P_data$cv_inds <- cut(seq(1,nrow(AE_P_data)),breaks=k,labels=FALSE)

#nre-order the data set
AE_P_data <- AE_P_data[order(as.numeric(row.names(AE_P_data))),]
cv_nr <- ncol(AE_P_data)
```
...
formula_rf <- as.formula(paste0(AE_P,"~",paste(explanatory_vars, collapse=" + ")))

n_features <- length(setdiff(names(Comp_dats), 'AE_P'))
...
for(l in 1:k){
  ...
  outer_tr <- AE_P_data[AE_P_data$cv_inds!=l,][,-cv_nr]
  outer_tst <- AE_P_data[AE_P_data$cv_inds==l,][,-cv_nr]
  ...
  outer_tst <- outer_tr[order(as.numeric(row.names(outer_tst))),]
    ...
    for(i in 1:nrow(prm_grd)){
      ...
      ms_RF[i,j] <- sqrt(mean((inner_tst[,"AE_P"]-inner_preds)^2))
    }
  }
  ...
  results[l,2] <- mean(outer_tr[,"AE_P"]-outer_pred_trn)
  results[l,3] <- sqrt(mean((outer_tr[,"AE_P"]-outer_pred_trn)^2))
  ...
  results[l,5] <- mean(outer_tst[,"AE_P"]-outer_preds)
  results[l,6] <- sqrt(mean((outer_tst[,"AE_P"]-outer_preds)^2))
  results[l,7] <- 1-((sum((outer_tst[,"AE_P"]-outer_preds)^2))/
             sum((outer_tst[,"AE_P"] - mean(outer_tst[,"AE_P"]))^2)) 
  
  # Dataframe with model predictions 
  QRF_predictions <- data.frame(outer_tst[,"AE_P"], outer_preds, outer_pred_tstQ$predictions)
  ...
  # outer_rmse[l] <- sqrt(mean((outer_tst[,"AE_P"]-outer_preds)^2))
  pp <- ggplot(QRF_predictions, aes(observed,predicted)) +
    ...
    labs(title = "", x = "Observed AE (P) (kg/kg)", y = "Predicted AE (P) (kg/kg)", color = "PICP_90") +
    coord_equal() +
    coord_equal() +
    ylim(-50,400) +
    xlim(-50,400) +
  ...
  ggsave(paste('AE_P', 'Fold', as.character(l),'.tiff', sep = '_'), path = "C:/Users/Eric_Asamoah/OneDrive - Wageningen University & Research/FERARI PhD 1 - Eric Asamoah/Academic/Data/Scripts/4_RF_models/Final_RF_Models/AE_P", width = 6, height = 4, device = "tiff", dpi = 300)
...
write.csv(results, "results_ae_p.csv", row.names = FALSE)

# Save list of best parameters in CSV file
write.csv(best_parameters, "best_parameters_ae_p.csv", row.names = FALSE)
```
...
saveRDS(final_ae_p_fit, "./final_ae_p_fit.rds") 
```

```{r}
# Variable importance plot from the optimal model
Var_imp_plt <- tibble(Covariate = names(final_ae_p_fit$variable.importance),
                      Importance = final_ae_p_fit$variable.importance,
                      Group = "") %>% 
  arrange(-Importance)
...
```{r}
ggsave(file = 'C:/Users/Eric_Asamoah/OneDrive - Wageningen University & Research/FERARI PhD 1 - Eric Asamoah/Academic/Data/Scripts/4_RF_models/Final_RF_Models/AE_P/aep_varimp1.png', width = 6, height = 4, dpi = 500)
```


```{r}
# Predictions for the training data
  outer_pred_fin <- final_ae_p_fit$predictions
  ...
  # Predictions for the testing data
  outer_pred_tstF <- predict(final_ae_p_fit,
                        Comp_dats, # just added this
                        type = "quantiles",
                        quantiles = QUANTILES,
                        keep.inbag = TRUE
                        )
  outer_pred_tst_F <- predict(final_ae_p_fit,
                        Comp_dats
                        )
  outer_preds_fin <- outer_pred_tst_F$predictions
  
# Compute n, ME, RMME and MEC for the testing phase
  Mean_F <- mean(Comp_dats[,"AE_P"]-outer_preds_fin)
  RMSE_F <- sqrt(mean((Comp_dats[,"AE_P"]-outer_preds_fin)^2))
  MEC_fin <- 1-((sum((Comp_dats[,"AE_P"]-outer_preds_fin)^2))/
             sum((Comp_dats[,"AE_P"] - mean(Comp_dats[,"AE_P"]))^2)) 
  
  # Dataframe with model predictions 
  QRF_predictions_fin <- data.frame(Comp_dats[,"AE_P"], outer_preds_fin, outer_pred_tstF$predictions)
  ...
  xlabtext <- expression(paste("Observed AE-P (kg kg"^"-1",")"))
  ylabtext <- expression(paste("Predicted AE-P (kg kg"^"-1",")"))
  
  png("AE--P.png",  width=2800, height=2800, res=600)
  ew <- hexbinplot(QRF_predictions_fin$predicted~QRF_predictions_fin$observed, colramp=colorRampPalette(pal),
             xlim=c(-50,400), ylim=c(-50,400),
  ...
  ggsave(paste('AEP_F', 'Fold', as.character(l),'.tiff', sep = '_'), path = "C:/Users/Eric_Asamoah/OneDrive - Wageningen University & Research/FERARI PhD 1 - Eric Asamoah/Academic/Data/Scripts/4_RF_models/Final_RF_Models/AE_P", width = 6, height = 4, device = "tiff", dpi = 300)
