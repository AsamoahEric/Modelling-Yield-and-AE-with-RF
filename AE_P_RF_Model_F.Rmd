...
k <- 5
# If the same number of folds in the outer and inner loop use:
p <- 10
...
for(l in 1:k){
  ...
  ggsave(paste('AE_P', 'Fold', as.character(l),'.tiff', sep = '_'), path = "./AE_P", width = 6, height = 4, device = "tiff", dpi = 300)
...
```{r}
ggsave(file = './aep_varimp1.png', width = 6, height = 4, dpi = 500)
```
...
# Compute n, ME, RMME and MEC for the testing phase
  ...
  ggsave(paste('AEP_F', 'Fold', as.character(l),'.tiff', sep = '_'), path = "./AE_P", width = 6, height = 4, device = "tiff", dpi = 300)
```
